...
int		main(void)
{
	...
	int		t;
	int 	D;			/* обьявляем переменные типа инт, котрые будут Результат деления и отсаток от деления */
	int		M;
	...
	ft_div_mod(i, t, &D, &M);		/* Вызываем функцию и отправляем туда делимое, делитель, и адреса переменных в котрые хотим записать рещультат деления и отсаток от деления */
	...
	ft_putnbr(i);			/* печатаем делитель t */
	ft_putchar(' ');		/* Печатаем симпол ' ' */
	ft_putnbr(i);			/* печатаем результат деления D */
	ft_putchar(' ');		/* Печатаем симпол ' ' */
	ft_putnbr(i);			/* печатаем делимое M */
