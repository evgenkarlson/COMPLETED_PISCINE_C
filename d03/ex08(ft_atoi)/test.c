...
int		ft_atoi(char *str)							/* Принимаем адрес массива символов */
{
	...
	/* проверим не лежат ли в этой ячейке массива ненужные символы если они там лежат то пропустим их и
	перейдем к след позиции */
	while ((str[i] == ' ') || (str[i] == '\t') || (str[i] == '\n')
		|| (str[i] == '\v') || (str[i] == '\f') || (str[i] == '\r'))
		i++;

	/* проверим лежит ли в след ячейке массива знака числа, символ '-' если он там то в переменной 
	negativ указываем то что это число отрицательное и переходим к след позиции*/
	if (str[i] == '-')
		negativ = 1;
	if ((str[i] == '-') || (str[i] == '+'))
		i++;

	/* Наконец дойдя до места в массиве где начинается число мы можем его вывести его от туда и сохранить
	в целочисленную переменную number */
	...
	/* Дальше перед тем как вернуть число которе мы добыли из строки нужно установить 
	для него тот знак который мы нашли */
