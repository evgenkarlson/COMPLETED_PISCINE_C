...
void	*ft_get_base(int nbr, char *base)
{
	...
	if (base_type)							/* проверяем массив с основанием числа, если он соответствует условиям то запускаем код внутри  */
	{
		...
		base_type = i;						/* Сохраним информацию о позиции лишней ячейки в переменной которая уже не будет использоваться чтобы сэкономить память */
		while (i > 0)						/* Запускаем цикл который запишет в final все найденные нами значения символов начиная с конца */
		{
			--i;
			final[i] = base[n[i]];			/* Для этого мы используем найденные нами значения, каждой цифры числа 'nbr' сохраненные в массива 'n', как шифр
		...
		final[base_type] = '\0';			/* Установим результирующий символ в конце нашего массива символов */
	}
	return (final);
}
...
int     main(void)									/* Основаная функция. Она принимает количество аргументов и массив со строками(массив указателей на указатели) */
{
	...
	ft_putstr(ft_convert_base(number, binary, hex));/* Запускаем функцию(ft_ultimate_range) которая получает массив чисел нужного нам диапазона и сохраняет его 
