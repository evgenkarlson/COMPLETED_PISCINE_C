...
{
	char			*str;				/* Обьявляем указатель который будет хранить данные о базовом типе нужной нам системы счисления */
	...
	str = "0123456789abcdef";			/* Инициализируем указатель заполняя его данными о базовом типе нужной нам системы счисления */
	while (j < 16 && i + j < size)		/* Запускаем цикл печати строки в hex формате. Её длинна должна быть не больше 16 символов и не должна выходить за рамки "size" */
	{
		...
		ft_putchar((char)str[(*(p + i + j) / 16) % 16]);/* Тут вычитываем шестнадцатиричное значение левой половины нужного нам символа */
		ft_putchar((char)str[*(p + i + j) % 16]);		/* Тут вычитываем шестнадцатиричное значение правой половины нужного нам символа */
		if (j % 2)										
			ft_putchar(' ');
		j++;
	}
