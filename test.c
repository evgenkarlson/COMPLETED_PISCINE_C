...
void		ft_putnbr_base(int nbr, char *base)
{
	...
	if (!heck_base(base))				/* проверяем массив с основанием числа, если он соответствует условиям то запускаем код внутри  */
	{
		...
		while (nbr)							/* пока значение переменной хранящей число не равно нулю */
		{
			...
			i++;							/* Увеличиваем счетчик для перехода к след ячейке. Так же переменная i нам понадобиться позже. 
											 ...
											 * чтобы напечатать символы в правильном порядке позже указывающий на количество найденых символов */
		}
		while (i > 0)						/* Запускаем цикл который напечатает все найденные нами и записанные в массив base символы начиная с конца */
			ft_putchar(base[n[--i]]);		/* Печатаем символ массива base начиная с конца массива пройдя к его началу */
	}
}
