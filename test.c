...
int     main(void)				/* Основаная функция */
{
	...
	int 	j;					/* Обьявляем счетчик для перемещения по выбранному массиву */
	...
	if ((arr_str = ft_split_whitespaces(" sebas  sdfds s sdf sdf chapuis wuid ")) == ((void *)0)) /* Запускам функцию нахождения в строке отдельных слов и разбрасываем ихв отдельные строки */
		return (0);
	while(arr_str[i])			/* Проверяем есть ли по этому адресу строка. Если есть то спускаемся ниже и печатаем ее символы */
	{
		j = 0;					/* Инициализируем счетчик для перемещения по массиву нулем чтобы начать с нулевой ячейки */
		while(arr_str[i][j])	/* Проверяем есть ли символы в текущей ячейке выбранного массива, если есть то печатаем их */
		{
			ft_putchar(arr_str[i][j]);/* Печатаем символ хранящийся в текущей ячейке массива */
			j++;				/* Переходим к след ячейке массива */
		}
		ft_putchar('\n');		/* После печати этого массива символов печатаем символ перехода на новую строку */
		i++;					/* И переходим к след строке хранящейся в массиве строк */
	}
