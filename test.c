...
char	*ft_strdup(char *src)	/* Функция дублирования строки. Она принимает в аргументах адресс массива символов строки. Дублирует ее и возвращает адресс дубликата */
{
	...
	l = ft_strlen(src);			/* считаем длинну полученой нами строки и сохраняем в переменную */
	if ((dest = malloc(sizeof(*src) * (l + 1))) == ((void *)0))	/* подсчитываем какого размера память нужно выделить, выделяем память и сохраняем ее адресс в dest */
