...
/*   Updated: 2020/09/26 00:36:27 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Нужно написать функцию, которая преобразует строковый аргумент str (системы 
счисления (N <= 16)) в целое число (основание 10) и возвращает его.

Символы, распознаваемые на входе: 0123456789abcdef
Они, конечно, должны быть урезаны в соответствии с запрошенной системой счисления. Для
Например, система счисления 4 распознает «0123», а система счисления 16 распознает «0123456789abcdef».

Прописные буквы также должны распознаваться: «12fdb3» совпадает с «12FDB3».

Знаки минус ('-') интерпретируется, только если он является первым символом строки.
