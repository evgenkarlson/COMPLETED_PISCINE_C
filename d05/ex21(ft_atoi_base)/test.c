...
int		ft_pow(int nb, int power)/* Функция возведения в степень */
{
	...
	if(power == 0)					/* Проверяем соответствует ли число в степени условиям */
		return (0);					/* Если не соответствует то завершаем функцию и возвращаем ноль */
	while (power--)				/* Запускаем цикл который умножит переменную 'r' на 'nb' столько раз сколько записано в 'power'. Возведет в 'nb' в степень 'power'. */
		result *= nb;			/* Умножая 'r' на 'nb' и сохраняя результат в той же переменной 'r' столько раз сколько записано в 'power'  
