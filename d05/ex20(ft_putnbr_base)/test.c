...
/*   Updated: 2020/09/23 11:38:56 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (base_type)							/* Проверяем массив с основанием числа, если он соответствует условиям то запускаем код внутри  */
	{
		...
		while (nbr)							/* Пока значение переменной хранящей число не равно нулю */
		{
			n[i] = nbr % base_type;			/* ВЫЧИСЛЯЕМ накаждой итерации цикла, то как выглядит каждый элемент текущего числа в нужной нам системе
											** счисления, с помощью 'base_type' (содержащего размер нужного нам базового типа).
			...
			nbr /= base_type;				/* Теперь для вычисления оставшихся чисел уменьшаем само число в nbr, разделив его на 
											** 'base_type' (размер нужного нам базового типа) и сохраняем результат в той же переменной nbr */
