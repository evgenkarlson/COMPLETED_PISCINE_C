...
/*   Updated: 2020/09/23 10:41:32 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
										...
										** базового типа. Если же массив не пройдет проверку на корректность, то функция вернет '0' */
{
										...
										** не повторяется ли символ в какой либо части текущего массива. */
	{
		...
		if (!((base[i] >= '0' && base[i] <= '9') || (base[i] >= 'a' \
				&& base[i] <= 'z') || (base[i] >= 'A' && base[i] <= 'Z'))) /* Провеяем каждый символ массива, чтобы там были только символы чисел
										** и букв алфавита */
		...
		while (base[z])					/* Запускаем цикл, который проверит все ячеки которые следуют за теми на которые указывает переменная 'i'
			...
			if (base[i] == base[z++])	/* Если символ в ячейке на которую указывает i  такой же что и в след ячейке */ 
				return (0);				/* То мы завершаем функцию и возвращаем '0' */ /* если нет то продолжаем цикл пробегаясь по остальным ячейка и сравнивая их с i */
