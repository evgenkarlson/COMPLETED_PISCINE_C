...
/*   Updated: 2020/09/30 23:34:26 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_hex(unsigned char *p, char *hex)/* Принимает адресс строки с которой будем работать и адрес строки с символами нужной нам системы счисления */
{
	unsigned int	i;					/* обьявляем счетчик отмеряющий пройденые символы */

	i = 0;								/* Инициализируем счетчик нулем чтобы начать печать символов с нулевой позици(относително переменной 'i') */
	while (i < 16)						/* Запускаем цикл печати строки в hex формате. Её длинна должна быть не больше 16 символов и не должна выходить за рамки "size" */
	{
		...
		ft_putchar((char)hex[(*(p + i) / 16) % 16]);/* Тут вычитываем шестнадцатиричное значение левой половины нужного нам символа */
		ft_putchar((char)hex[*(p + i) % 16]);		/* Тут вычитываем шестнадцатиричное значение правой половины нужного нам символа */
		if (i % 2)										
			ft_putchar(' ');
		i++;
	}
	while (i < 16)						/* Если j по прежнему меньше 16 значит мы достигли предела печати символов а место еще осталось и мы заполняем оставшееся пробелами */
	{
		ft_putchar(' ');
		ft_putchar(' ');
		if (i % 2)
...
/* Функция печати строки длинной в 16 символов */
 void	ft_print_char(unsigned char *p)			/* Принимает адресс строки с которой будем работать */
 {	
	unsigned int	i;							/* Обьявляем счетчик отмеряющий пройденые символы */

	i = 0;
	while (i < 16)								/* Запускаем цикл печати той же строки что и выше но уже в стандартном формате 
												** Её длинна должна быть не больше 16 символов */
	{
		if (*(p + i) >= 32 && *(p + i) <= 126)	/* Если символ являеться печатаемым */
			ft_putchar((char)*(p + i));			/* ТО печатаем его */
		else
			ft_putchar('.');					/* Если символ не печатаемый то печатаем вместо него точку */
		i++;									/* переходим к след ячейке массива символов(строки) */
	}
 }
...
void	*ft_print_memory(void *addr, unsigned int size) /* Принимает в параметрах адресс и число которое покажет сколько мы хотим напечатать */
{
	...
	while (i < size)					/* Запускаем цикл с каждой итерацией которого мы печатаем новую строку */
	{
		...
		ft_print_hex(p + i, hex);		/* Запускаем печать строки в hex. Отправляя позицию нужного нам первого символа, общий предел печати(size) и адресс строки, символы для hex */
		ft_print_char(p + i);			/* Запускаем печать строки как есть. Отправляя туда позицию нужного нам первого символа, общий предел печати(size) и адресс строки */
