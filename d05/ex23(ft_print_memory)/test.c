...
/*   Updated: 2020/09/30 23:38:39 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_hex(unsigned char *p, char *hex)/* Принимает адресс строки с которой будем работать и адрес строки с символами нужной нам системы счисления */
{
	...
	while (i < 16)						/* Если i по прежнему меньше 16 значит мы достигли предела печати символов а место еще осталось и мы заполняем оставшееся пробелами */
	{
		ft_putchar(' ');
		ft_putchar(' ');
		if (i % 2)
...
void	*ft_print_memory(void *addr, unsigned int size) /* Принимает в параметрах адресс и число которое покажет сколько мы хотим напечатать */
{
	...
	while (i < size)					/* Запускаем цикл с каждой итерацией которого мы печатаем новую строку */
	{
		...
		ft_print_hex(p + i, hex);		/* Запускаем печать строки в hex. Отправляя туда туда адресс следующей строки для печати и символы для hex */
		ft_print_char(p + i);			/* Запускаем печать строки как есть. Отправляя туда адресс следующей строки для печати */
