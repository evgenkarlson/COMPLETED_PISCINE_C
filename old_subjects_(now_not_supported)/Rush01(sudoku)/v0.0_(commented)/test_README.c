...
int		check_block(char **tab, int y, int x, char nb)/* Принимаем массив строк, координаты строки, координаты столбика, символ для проверки в кубе 3x3 */
{
	...
	if (y <= 3)									/* Высчитываем принадлежность 'y' к конкретному кубу. И центрируем точку отсчета 'x' на начало этого куба */
	...
	x = x - (x % 3);							/* Высчитываем принадлежность 'x' к конкретному кубу из 9. И устанавливаем точку отсчета 'x' на начало этого куба */
