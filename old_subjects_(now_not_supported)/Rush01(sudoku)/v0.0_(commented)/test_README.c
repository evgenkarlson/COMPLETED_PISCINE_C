...
int		check_block(char **tab, int y, int x, char nb)/* Принимаем массив строк, координаты строки, координаты столбика, символ для проверки в кубе 3x3 */
{
	...
	if (y <= 3)									/* Высчитываем принадлежность 'х' к конкретному кубу. И центрируем точку отсчета 'x' на начало этого куба */
		y = 1;									/* если диапазон от 1 до трех то центрируем на 1; */
	else if (y <= 6)							/* если диапазон от 4 до 6*/
		y = 4;									/* то центрируем на 4 */
	else if (y <= 9)							/* если диапазон от 7 до 9 */
		y = 7;									/* то центрируем на 7 */
	x = x - (x % 3);							/* Высчитываем принадлежность 'y' к конкретному кубу из 9. И устанавливаем точку отсчета 'y' на начало этого куба */
	while (i < 3)								/* запускаем цикл который пройдется по каждой строке куба 3x3 */
	{
		...
		while (j < 3)							/* запускаем цикл который пройдется по каждой ячейке каждой строки куба 3x3 */
		{
			...
			x++;								/* если похожего символа не нашли проверяем дальше пока не дойдем до конца строки */
											...
											** которого будут высчитываться координаты отсчета для вычисления */
{
	...
	y = position / 9;						/* установим координаты отсчета по горизонтали направо*/ /*так мы узнаем то какая строка нам нужна.*/
	x = position % 9;						/* установим координаты отсчета по вертикали вниз*/ /*так мы узнаем какой символ в этой строке нужен.*/
	...
	while (++nb <= '9')
	{
		if (check_line(tab, nb, y) + check_column(tab, nb, x) == 2) /* проверяем есть ли совпадение этого число по горизонтали и вертикали */
		{
			if (check_block(tab, y, x, nb) == 1)	/* если все отлично и таких совпадений нет то проверяем еще нет ли такого же числа в самом блоке(3x3) */
			{
