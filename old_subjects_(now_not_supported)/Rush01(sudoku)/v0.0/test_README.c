...
/*   Updated: 2020/10/19 15:02:24 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_block(char **tab, int x, int y, char nb)/* Принимаем массив строк, координаты строки, координаты столбика, символ для проверки в кубе 3x3 */
{
	int i;										/* обьявляем переменную для счетчика координат 'x' */
	int j;										/* обьявляем переменную для счетчика координат 'y' */

	i = 0;										/* инициализируем счетчик количества пройденых строк куба 3x3 */

	if (x <= 3)									/* Высчитываем принадлежность 'х' к конкретному кубу. И центрируем точку отсчета 'x' на начало этого куба */
		x = 1;									/* если диапазон от 1 до трех то центрируем на 1; */
	else if (x <= 6)							/* если диапазон от 4 до 6*/
		x = 4;									/* то центрируем на 4 */
	else if (x <= 9)							/* если диапазон от 7 до 9 */
		x = 7;									/* то центрируем на 7 */

	y = y - (y % 3);							/* Высчитываем принадлежность 'y' к конкретному кубу из 9. И устанавливаем точку отсчета 'y' на начало этого куба */
	while (i < 3)								/* запускаем цикл который пройдется по каждой строке куба 3x3 */
	{
		j = 0;									/* инициализируем счетчик количества пройденых ячеек куба 3x3 */
		while (j < 3)							/* запускаем цикл который пройдется по каждой ячейке каждой строки куба 3x3 */
		{
			if (tab[x][y] == nb)				/* если похожий символ уже существует в кубе значит нужно искать другой */
				return (0);						/* функция завершается возвращаая 0 */
			y++;								/* если похожего символа не нашли проверяем дальше пока не дойдем до конца строки */
			j++;								/* увеличиваем для условия число показывающее количество проверенных ячеек на этой строке */
		}
		y = y - 3;
		x++;									/* если похожего символа на этой строке не нашли переходим к след строке куба и проверяем
												 * дальше пока не дойдем до конца */
		i++;									/* увеличиваем для условия число показывающее количество проверенных строк в кубе 3x3 */
	}
...
int	check_column(char **tab, char nb, int y)	/* Принимаем массив строк, символ для проверки, координаты столбика */
{
	...
	{
		if (tab[i][y] == nb)					/* если похожий символ уже существует значит нужно искать другой */
			return (0);							/* функция завершается возвращаая 0 */
		i++;									/* если похожего символа не нашли проверяем дальше пока не дойдем вниз до конца столбика */
	}
...
int	check_line(char **tab, char nb, int x)
{
	...
	i = 0;										/* инициализируем ее нулем т.к нужые нам ячейки с символами начинаются с нулевой позиции и до 8 включительно */
	...
	{
		if (tab[x][i] == nb)					/* если похожий символ уже существует значит нужно искать другой */
			return (0);							/* функция завершается возвращаая 0 */
		i++;									/* если похожего символа не нашли проверяем дальше пока не дойдем до конца строки */
	}
	return (1);									/* Если похожего символа по горизонтали нет значит все отлично, возвращаем 1 */
}
...
а их вычислять мы будем так: 
	x = position / 9;			так мы узнаем то какая строка нам нужна.
	y = position % 9;			так мы узнаем какой символ в этой строке нужен.
Так мы вычислим позицию первого символа первой строки.
Потом мы увеличиваем position на один и снова вычисляем 'x' и 'y' уже из нового
...
position 81 = array[9][0]; position 82 = array[9][1]; position 83 = array[9][2]; 
position 84 = array[9][3]; position 85 = array[9][4]; position 86 = array[9][5]; 
position 87 = array[9][6]; position 88 = array[9][7]; position 89 = array[9][8]
											 ...
											 * которого будут высчитываться координаты отсчета для вычисления */
{
	int		x;								/* обьявим x который будет хранить координаты отсчета по горизонтали (position / 9 = 1)
											 * в 'x' мы будем хранит координаты нужной строки */

	int		y;								/* обьявим y который будет хранить координаты отсчета по вертикали (position % 9 = 0)
											 * в 'y' мы будем хранит координаты нужного символа
											 * все вместе будет выглядеть так tab[x][y] */
...
{
	if (check_grille(argv, argc))	/* (Шаг 2)проверяем решетку с символами если все норм продолжаем */
	{
		if (sudoku(argv, 9))		/* (Шаг 3)Переходим к решению Судоку, если все норм идем дальше 
		...
		{
			ft_putstr("Error");		/* Если судоку не решилось выводим сообщение */
			ft_putchar('\n');
		}
	}
	else
	{
		ft_putstr("Error");			/* Если решетка проверку не прошла выводим сообщение */
		ft_putchar('\n');
	}
	return (0);
}
