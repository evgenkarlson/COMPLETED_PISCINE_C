 ...
 * появляются в обеих строках, в порядке, в котором они появляются в первой. */
void	ft_inter(char *s1, char *s2)				/* Принимаем адреса строк и записываем их в указатели */
{
	...
	{
		if (ft_verif_char(s1, s1[i], i) && !ft_verif_char(s2, s2[i], -1))	/* Проверяем. если в первом массиве ранее таких символов
...
/* Функция проверки наличия символа в массиве пока счетчик не равен индексу */
int		ft_verif_char(char *str, char c, int index)	/* Принимаем адресс массива, символ, и порядковый номер символа(его индекс) */
{
	int		i;										/* обьявим переменную для счетчика */
