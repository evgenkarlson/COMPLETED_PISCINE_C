...
/*   Updated: 2020/10/11 23:56:06 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_getnbr_base(int nbr, char *base)
{
											...
											** то в переменной должно находиться положительное число. Если число положительно, то мы запускаем код внутри  */
	{
		...
		j = 0;								/* Используем счетчик для перемещения по финальному массиву, инициализируя ее нулем чтобы 
...
char	*ft_convert_base(char *nbr, char *base_from, char *base_to)/* Принимаем в аргументах строку с числом, прошлую систему счисления и желаемую систему счисления */
{
	...
	final = ft_getnbr_base(dec, base_to);	/* Вызовем функцию преобразования десятичного числа в строку с числом определенной системы счислени и полученый адрес сохраним в "final" */
...
int     main(void)							/* Основаная функция */
{
	...
	result = ft_convert_base(number, binary, hex);/* Запускаем функцию(ft_convert_base) которая берет строку с числом и преобразовывает его из текущей системы счисления
