...
void	ft_cat(int argc, char **argv)		/* Принимаем в 'argc' количество строк, и адресс массива строк в '**argv' */
{
	...
	g_progname = *argv;						/* Инициализируем глобальный указатель адресом строки хранящей имя програмы */
	i = 0;									/* Инициализируем счетчик */
	while (++i < argc)						/* Увеличиваем счечик 'i' и проверяем условие, если счетчик меньше 'argc' запускаем цикл 
											** последовательно напечатает содержимое каждого файла */
	{
	...
	if (argc == 1)							/* Если массив строк содержит только имя программы то печатаем содержимое самого файла программы */
		ft_display_file(g_progname);		/* Отправляем адресс строки, содержащей имя и путь до самой программы, в функцию для печати её содержимого */
}