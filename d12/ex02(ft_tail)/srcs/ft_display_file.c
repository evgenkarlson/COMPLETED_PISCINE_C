...
/*   Updated: 2020/09/22 03:09:04 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_display_file(int i, int fd, int argc, char **argv)
{
	...
	int		rnum;											/* Обьявляем переменную для получения количества считанных из файла данных или же для хранения ошибок чтения файла */
	...
	if (argv[1][0] == '-' && (argv[1][1] == 'C' || argv[1][1] == 'c'))	/* Проверяем аргументы программы, если все хорошо запускаем код внутри */
	{
															...
															** на ту позицию которая нам нужна чтобы напечатать строго определенное количество байт с конца файла */
		{
			...
			readed += rnum;									/* Если ошибки чтения нет, то записываем количество считанных данных и переходим к след. итерации */
		}
		...
		{
			while  ((rnum = read(fd, &buffer, 1)))			/* Запускаем цикл, который с помощью функции 'open' сразу из условия будет порциями считывать данные из файла 
															** во временный буфер используя дескриптор файла(идентификатор потока к файлу), перменную 'buffer' и её размер.
															...
															** Цикл проверит содержимое переменной 'rnum' и если он увидит что функция 'open' вернула некоторое число, то он 
															** запустит код внутри себя, который проверит это число, и если все хорошо, напечатает содержимое буфера */
			{
				if (rnum == -1)								/* Проверяем число которое нам вернула функция 'open'. Если это число равно '-1' значит у нас произошла ошибка чтения */
				{
			...
			if (i < (argc - 1))								/* Проверяем, если мы напечатали содержимое последнего файла принятог в аргументах приложения */
				ft_putchar('\n');							/* То печатаем символ перехода на новую строку  и завершаем функцию*/
		}
	}
}