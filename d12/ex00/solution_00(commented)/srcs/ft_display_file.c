											...
											** путь до самого файла для печати, и печатает содержимое этого файла */
{
	...
	char	buffer[BUFF_SIZE + 1];			/* Обьявляем массив символов(строку), размером 'BUFF_SIZE' +1(для хранения символа завершающего нуля).
											** Этот массив мы используем как буффер для считывания в него порциями данных из потока, на который указывает
											** идентификатор файлового потока(дескриптор файла), который будет хранится в переменной 'fd' */
	...
	else if (argc == 2)						/* Если же количество аргументов соответсвует требованиям то мы запускаем код ниже */
	{
		...
		{
			while ((r = read(fd, buffer, BUFF_SIZE)))/* Запускаем цикл, который с помощью функции 'open' сразу из условия будет порциями считывать данные из файла 
												...
												** запустит код внутри себя, который проверит это число, и если все хорошо, напечатает содержимое буфера */
			{
				...
				{
					buffer[r] = '\0';			/* Перестрахуемся и установим после считанных данных символ завершающего нуля для корректности печати */
			...
			if (close(fd) == -1)				/* Закрываем файловый поток с помощью функции 'close' и дескриптора файла(идентификатора файлового потока).
				...
				write(1, "\n", 1);				/* Печатаем символ перехода на новую строку для корректного отображения */
		}
	}
}
