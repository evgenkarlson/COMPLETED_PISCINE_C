...
/*   Updated: 2020/11/16 16:48:14 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_list_reverse(t_list **begin_list)	/* Функция переворачивает задом на перед цепь связанных между собой экземпляров структур */
{
	t_list	*new_start;					/* Обьявляем указатель для хранения в нем адреса первого экземпляра новой цепи частично перевернутых
										** на данный момент и связанных между собой экземпляров структур */
	t_list	*tmp;						/* Обьявляем указатель для сохранения туда адреса представляющего оставшуюся часть не перевернутой напи цепи 
										** экземпляров структур, для ипользования его в следующиих итерациях цикла разворачивания цепи экземпляров
										** структур */

	if (*begin_list || ((*begin_list)->next))/* Проверяем являеться ли полученый адресс экземпляра структуры проинициализированнным и содержит
										** ли экзепляр структуры адрес другого экземпляра структуры для переворачивания этой цепи связанных между
										** собой структур */
	{
		new_start = ((void *)0);		/* Инициализируем указатель, сохраняя в него нулевой адресс для "закупоривания первого экземпляра структуры
										** в цепи связанных между собой структрур, чтобы переместить его в конец новой, перевернутой цепи и он 
										** не ссылался дальше на др. экземпляры т.к будет являеться последним в новой перевернутой цепи связанных 
		...
		while (*begin_list)				/* Запускаем цикл, который перевернет цепь связанных между собой экземпляров структур */
		{
			...
			(*begin_list)->next = new_start;/* Перезаписываем указатель хранящий адрес следующего экземпляра используя сохраненный, с прошлой итерации
										** цикла, адрес экземпляра структуры, который сейчас представляет начало выстроеной цепочки уже 
										** частично перевернутой цепи экземпляров структур */
			new_start = *begin_list;	/* Перезаписываем указатель, хранящий адрес первого экземпляра частично перевернутой цепи экземпляров структур,
										** на, дополненый еще одним экземпляром, вариант цастично перевернутой цепи экземпляров структур */
			*begin_list = tmp;			/* Презаписываем указатель сохраняя туда адрес представляющий оставшуюся часть, еще не перевернутой напи, цепи 
										** экземпляров структур, для ипользования его в следующиих итерациях цикла переворачивания цепи экземпляров
										** структур */
		}
		*begin_list = new_start;		/* Записываем адрес экземпляра структуры(который стал последним элементом перемещенным в начало цепи
										** связанных между собой экземпляров структуры в результате разворацивания этой цепи задом на перед) 
										** в указатель хранящий адрес первого экземпляра структуры, цепи связанных между собой экземпляров структур */
	}
}
...
int			main(void)
{
	...
	ft_list_reverse(&list);				/* Отрпавляем адрес указателя хранящего адрес первого экземпляра структуры цепи связанных между собой 
										** экземпляров структур в функцию которая первернет эту цепь задом на перед */
	ft_put_list(list);					/* Запускаем функцию печати данных каждого экземпляра структуры цепи связанных между собой экземпляров структур*/
