...
/*   Updated: 2020/09/28 19:34:26 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_list_reverse(t_list **begin_list)	/* Функция переворачивает задом на перед цепь связанных между собой экземпляров структур */
{
										...
										** собой структур */
	{
		...
		{
			tmp = (*begin_list)->next;	/* Сохраняем адрес следующего экземпляра структуры */
			...
			*begin_list = tmp;			/* Презаписываем указатель сохраняя туда адрес представляющий оставшуюся часть не перевернутой напи цепи 
										** экземпляров структур, для ипользования его в следующиих итерациях цикла разворачивания цепи экземпляров
										** структур */
		}
