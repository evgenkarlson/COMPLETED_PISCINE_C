...
t_list	*ft_list_push_params(int ac, char **av) /* Функция разбирает массив строк и заполняет каждой из строк экземпляр структуры типа 't_list' соединяя их 
										 * между собой в цепь связанных между собой структур в порядке убывания. Т.Е. так чтобы экземпляры структур,
