...
/*   Updated: 2020/09/28 19:33:40 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list		*ft_list_push_params(int ac, char **av) /* Функция разбирает массив строк и заполняет каждой из этих строк экземпляры структуры типа 
										 ...
										 * оказались первыми в цепи связанных между собой экземпляров структуры типа 't_list' */
{
	...
	tmp = ft_create_elem(av[0]);		/* Создадим первый экземпляр структуры, записав адресс нулевой строки массива строк 'av' в указатель 'data',
	...
	while (i < ac)						/* Запускаем цикл который создаст цепь из связанных между собо экземпляров структур, каждый из которых мы по
										 * очереди заполним адресом строки массива строк 'av', до тех пор пока мы не дойдем до конца массива строк */ 
	{
		list = ft_create_elem(av[i]);	/* Создадим следующий экземпляр структуры, записав в одном из его элементов адресс текущей строки массива 
										 * строк 'av' */
		...
		tmp = list;						/* А адрес текущей структуры мы сохраним во временном указателе чтобы потом сохранить его в элементе 'next' 
										 ...
										 * цепи связанных между собой экземпляров структуры типа 't_list' хранящий строки массива строк 'av' */
		i++;							/* Увеличиаем счетчик для перехода к след строке массива строк 'av' и инициализации с помощью него нового
										 * экземпляра структуры. И так до тех пор пока строки масива строк 'av' не закончатся */
	}
...
int			main(int argc, char *argv[])
{
	...
	{
		ft_put_list(ft_list_push_params(argc, argv)); /* Запустим функцию которая найдет последний экземпляр структуры среди связанных между собой
