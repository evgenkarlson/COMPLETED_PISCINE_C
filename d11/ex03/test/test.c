...
void	ft_put_struct(t_list **tab_list)	/* Функция печатает данные одного из элементов каждого экземпляра связаных между собой вглубь  
											 * экземпляров */
{
	ft_putstr((char *)((*tab_list)->data));	/* Приводим получаемый адрес к типу 'char' и отравляем на печать. Так мы печатаем данные одного  
											 * из элеменетов первого экземпляра структуры */
	while((*tab_list)->next)				/* Запускаем цикл, который пройдется вглубь связаных между собой экземпляров структур и напечатает  
											 * данные одного из элеменетов каждого экземпляра связаных между собой структрур */
	{
		ft_putstr((char *)((*tab_list)->next->data));/* Приводим получаемый адрес к типу 'char' и отравляем на печать. Так мы печатаем данные  
													  * одного из элеменетов текущего экземпляра структуры */
		(*tab_list)->next = (*tab_list)->next->next; /* Для того чтобы дойти до самого последнего эземпляра структуры типа 't_list' мы cохраняем
													  * в указателе предыдущей структуры типа t_list адрес хранящийся в одном из элементов(указатель
													  * структуры типа t_list) текущего экземпляра структуры типа t_list */
...
void	ft_list_push_front(t_list **begin_list, void *data)	/* Функция, которая в первом экземпляре структуры, среди связаных между собой 
										 * последовательно экземпляров структур, создаст экземпляр структуры типа t_list и сохранит его 
										 * адрес в элементе(указатель на экземпляр структуры типа 't_list') первого экземпляра структуры(который 
										 * является указателем на экземпляр структуры типа t_list) */
{
	t_list	*list;						/* Обьявим указатель структуры типа t_list для хранения адреса экземпляра структуры типа t_list */

	if (*begin_list)					/* Проверяем хранится ли в 'указателе на указатель' принятом в рагументах функции адресс 'экземпляра 
										 * структуры типа t_list' */
	{
		...
		list->next = *begin_list;		/* Адресс экземпляра структуры типа t_list мы сохраняем в одном из элементов(указателе на экземпляр 
										 * структуры типа 't_list') 
										 * созданного нами экземпляра структуры чтобы сдвинуть позицию текущей структуры среди связаных между
										 *  собой последовательно 
										 * экземпляров структур, на второе место */
		*begin_list = list;				/* В указателе на экземпляр структуры типа 't_list' сохраняем адресс созданого нами новый экземпляр 
										 * структуры типа 't_list' */
	}
	else									/* Если мы дошли до сюда значит в 'указателе' принятом в рагументах функции не хранится адрес
											 * экземпляра структуры типы 't_list' */
		*begin_list = ft_create_elem(data);	/* Тогда, мы просто создаем экземпляр структуры типа 't_list' и сохраняем его адресс в указателе
											 * на экземпляр структуры типа 't_list' */
}

/* ************************************************************************** */

int	ft_list_size(t_list *begin_list)	/* Функция внутри текущей структуры считает количество структур связанных между собой последовательно */
{
	...
	while (begin_list)					/* Зпускаем цикл который пройдется вглубь цепочки связанных между собой экземпляров структуры типа 't_list' */
	{
...
int			main(void)
{
	...
	ft_putnbr(ft_list_size(*tab_list));
