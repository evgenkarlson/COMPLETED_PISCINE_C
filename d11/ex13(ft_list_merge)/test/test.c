...
{
	t_list	*list_ptr;					/* Обьявим указатель для хранения адреса текущего экземпляра структуры цепи связанных между собой экземпляров */

	if (*begin_list1)					/* Если указатель с первым экземпляром цепи связанных между собой экземпляров не пуст то запускаем код, который 
										 * найдет конец первой цепи связяанных между собой экземпляров и свяжет конец этой цепи со второй цепю связанных
										 * между экземпляров */
	{
		list_ptr = *begin_list1;		/* Сохраняем адрес первого экземпляра первой цепи во временный указатель для прохождения по адресам экземпляров */
		while (list_ptr->next)			/* ЗАпускаем цикл который пройдеться по всем экземплярам структуры цепи связаннх между собой экземпляров,
										 * до тех пор пока мы не втретим экземпляр структы, который не содержит адреса на след экземпляр структуры */
			list_ptr = list_ptr->next;	/* Перезаписываем указатель хранящий адресс текущего экзепляра структуры на адресс следующего экзепляра структуры
										 * для работы в след итерации цикла со следующим экзепляром структуры */
		list_ptr->next = begin_list2;	/* Сохраняем в указателе найденого нами последнего экземпляра структуры адрес первого экземпляра второй цепи связанных
...
int			main(void)
{
	...
	ft_list_merge(&list1, list2);		/* Помещаем элементы списка list2 в конец другого списка list1 соединяя их в одну цепь связанных между собой структур */
	ft_put_list(list1);
