										 ...
										 * соединяя их в одну цепь связанных между собой структур  */
{
										 ...
										 * цепью связанных между собой экземпляров */
	{
		...
		while (list_ptr->next)			/* ЗАпускаем цикл который пройдеться по всем экземплярам структуры цепи связаннх между собой экземпляров,
			...
			list_ptr = list_ptr->next;	/* Перезаписываем указатель хранящий адресс текущего экзепляра структуры на адресс следующего экзепляра 
										 * структуры для работы в след итерации цикла со следующим экзепляром структуры */
		list_ptr->next = begin_list2;	/* Сохраняем в указателе найденого нами последнего экземпляра структуры адрес первого экземпляра второй цепи 
