	...
	while (current)						/* Запускаем цикл, который будет работать до тех пор пока не дойдет до конца цепи связанных между собой структур */
	{
		if ((*cmp)(current->data, data_ref) == 0)	/* Если содержимое текущей струтуры такое же как и содержимое по адресу указателя *data_ref */
		{
			if (current == *begin_list)				/* Если найденый экземпляр структуры расположен первым в цепи связанных между собой экземпляров */
				*begin_list = current->next;		/* То перезаписываем указатель хранящий адрес текущего экземпляра цепи на адресс следующего экземпляра цепи */ 
			else									/* Если найденый экземпляр структуры не первый в цепи связанных между собой экземпляров */
				last->next = current->next;			/* То перезаписываем указатель хранящий адрес текущего экземпляра цепи на адресс следующего экземпляра цепи */
			tmp = current;				/* Сохраняем во времененый указатель адресс найденого экземпляра структуры для освобождения памяти */
			current = current->next;	/* Перезаписываем указатель хранящий адресс текущего экзепляра структуры на адресс следующего экзепляра структуры
		...
		else							/* Если содержимое текущей струтуры не такое же как и содержимое по адресу указателя *data_ref */
		{
			...
			current = current->next;	/* Перезаписываем указатель хранящий адресс текущего экзепляра структуры адресс следующего экзепляра структуры
										 * для работы в след итерации цикла со следующим экзепляром структуры */
		}
	}
}
