										 ...
										 *  «begin_list1», так что «begin_list1» остается отсортированным в порядке возрастания*/
{
	...
	if (!*(begin_list1))				/* Если указатель '*begin_list' хранящий начало первой цепи связанных между собой экземпляров структур не 
										 * содержит адрес экземпляра структуры, значит первой цепи связанных между собой экземпляров не существует 
										 * то мы запускаем код ниже*/
	{
		if (!begin_list2)				/* Если и указатель 'begin_list2' хранящий начало второй цепи связанных между собой экземпляров структур не 
										 * содержит адрес экземпляра структуры, значит второй цепи связанных между собой экземпляров не существует
										 * то мы завершаем функцию */
			return ;
		else							/* В другом случае если указатель 'begin_list2' хранящий начало второй цепи связанных между собой экземпляров
										 * структур содержит адрес экземпляра структуры, значит вторая цепь связанных между собой экземпляров
										 * существует и мы  помещаем адресс экземпляра структуры являющийся началом второй цепи 'begin_list2' 
										 * в указатель хранящий начало первой цепи связанных между собой экземпляров структур */
		{
			*begin_list1 = begin_list2;	/* Значит мы сохраняем в указателе, который должен хранить адрес первого экземпляра цепи 'begin_list1' адрес
										 * первого экземпляра цепи 'begin_list2' */
			return ;					/* Завершаем функцию */
		}
	}

	else								/* Если у нас все таки есть обе цепи связанных между собой экземпляров структур то чтобы обьединить их в одну
	 									 * цепь связанных между собой экземпляров структуры мы будем искать последний экземпляр структуры первой цепи
										 * 'begin_list1' чтобы записать в его указатель адрес первого экземпляра структуры цепи 'begin_list2' */
	{
