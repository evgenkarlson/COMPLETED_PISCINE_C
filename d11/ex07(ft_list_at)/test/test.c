...
int			main(void)
{
	...
	tmp = ft_list_at(list, 3);			/* Оправляем в функцию 'ft_list_at' адресс первого экземпляра структуры цепи связанных между собой последовательно
										 * экземпляров структур и число отражающее позицию нужного нам экземпляра стурктуры, который находится в этой цепи
										 * связанных между собой последовательно экземпляров структур.
