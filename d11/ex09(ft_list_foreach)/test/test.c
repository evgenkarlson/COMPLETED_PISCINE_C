...
void		ft_putstr(void *str)			/* Функция принимает адрес неизвестного типа но предпологаая что там находится строка переводит адрес в тип
...
void		ft_list_foreach(t_list *begin_list, void (*f)(void *))/* функция принимает адресс первого экземпляра структуры уепи связанных между собой 
										 * структур и применяет для каждого экземпляра структуры этой цепи функцию, адресс которой мы приняли в 
										 * аргументах */
{
