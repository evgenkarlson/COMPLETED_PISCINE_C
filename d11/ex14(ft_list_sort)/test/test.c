										 ...
										 * экземпляров структур */
		{
			if (current == *begin_list)	/* Если адрес экземпляра в указателе 'current' такой же как и адрес в указателе '*begin_list', значит экземпляры
				...
				last = *begin_list = (*begin_list)->next; /*Для начала мы перезаписываем адрес в указателе '*begin_list' на адрес следующего экземпляра 
										 ...
										 * сохраняем в указателе 'last' перезаписывая его для использования этого адреса в след итерации цикла для сортировки
										 * экземпляров, которые будут находится в середине цепи связанных между собой экземпляров */

			else						/* Если адрес экземпляра в указателе 'current' НЕ такой же как и адрес в указателе '*begin_list', значит экземпляры
										 * которые мы сейчас хотим поменять местами находятся НЕ в начале а в где то СЕРЕДИНЕ цепи связянных между собой 
										 * экземпляров структур. */
				last = last->next = current->next; /* Тогда мы используем адрес, который мы сохранили в прошлой итерации цикла во временный указатель 
										 * 'last'. Этот указатель хранит адрес предыдущего экземпляра и с помощью него мы сможем поменять местами текущий
										 * и следующий экземпляр. 
										 ...
										 * собой экземпляров */
			tmp = current->next->next;	/* Чтобы не потерять всю последующую цепь связанных между собой структур при дальнейшейм завершениии сортировки
										 * этих двух экземпляров мы сохраняем во временный указатель 'tmp' адрес третьего экземпяра структуры(третий адрес
										 * относительно адреса текущего экземплятора) */
			current->next->next = current; /* Теперь, когда мы сдвинули "второй" экземпляр на позициию ниже, следом за ним мы запишем "первый" экземпляр. 
											* А реализуем мы это так - запишем в указатель сдвинутого ниже "второго" экземпляра адресс "первого" экземпляра */
			current->next = tmp;		/* А в указатель "первого" экземпляра который тееперь на второй позиции мы запишем сохраненный во временном указателе
										 * адрес "третьего" экземпляра */
			current = *begin_list;		/* Сбрасываем на новое начальное значение указатель хранящий адрес экземпляра с которым мы сейчас работаем
										 * чтобы начать сортировку с самого начала цепи связанных между собой структур и так до тех пор пока не закончим 
										 * сортировку */
		}
