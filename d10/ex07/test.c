...
#include <stdlib.h>				/* Подкоючаем библиотеку содержащую функцию для выделения памяти(malloc) и функцию для подсчета размера (sizeof) */
...
void	ft_swap(char **x, char **y)	/* Функция меняет местами адреса в указателях на указатели */
{
	char *temp;
...
int		getlength(char **tab)		/* Функция проходится по элементам массива указателей и считает их количество  */
{
	...
	while (tab[i] != 0)				/* Запускаем цикл, который пройдется по каждому элементу массива указателей, до тех пор пока они не закончатся */
...
void	ft_sort_wordtab(char **tab)	/* Функция сортирует массив со строками по возростанию */
{
									 ...
									 * цикла будет перемещать один самый больший элемент массива строк в конец */
	{
		...
		{
			if (ft_strcmp(tab[j], tab[j + 1]) > 0)	/* Если текущая строка больше следующей */ 
			{
				ft_swap(&tab[j], &tab[j + 1]);		/* То меняем их местами */
			}
