...
void	ft_putnbr(int nb)     			 /* Функция вывода числа */
{
	...
	{
		ft_putchar((char)((temp / size)) + 48);			/* делим число на размер котрый у нас получился */   /* тут тоже хитрость */  
		...
		size /= 10;					/* после то как отделили 3 от 45 в числе 345 и после того как оставили себе только 45 для дальнейшего отделения число 100 нам больше не нужно
									так что делим его на 10 чтобы в след итерации 45 уже делилось на 10 отдавая на 4 на печать, а 45 делилось по модулю на 10 и сохраняя 5 
									на следующую итерацию, потом снова уменьшение size на 10 чтобы можно было работать с чсло которое теперь уже хранит 5. 
