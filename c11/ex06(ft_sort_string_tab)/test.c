...
/*   Updated: 2020/09/28 19:31:08 by evgenkarlson     ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sort_string_tab(char **tab)	/* Функция сортирует массив со строками по возростанию */
{
									 ...
									 * цикла будет перемещать один самый больший элемент массива строк в конец */
	{
		...
		{
			if (ft_strcmp(tab[j], tab[j + 1]) > 0)	/* Если символы текущей строки больше чем символы следующей */ 
			{
...
int		main(void)
{
	...
	ft_sort_string_tab(point);				/* Отправим добытый из прошлой функции адресс массива строк в функцию,
										 * которая отсортирует нам эти строки в порядке возрастания */
	ft_putstr_tab(point);				/* Напечатаем результат */
